...
To understand why this handbook is structured the way it is, you need to understand what a real shift looks like. In Volume 5, you are the final point of escalation.

**02:00 AM — Shift Begins**
You log into the ticketing system. PagerDuty fires: *"Global Checkout API latency spike."*

**02:02 AM — The Investigation**
The Mid-Level engineers have been fighting it for an hour. They restarted the pods, but it didn't help. They suspect the database, but query logs are clean. You take Incident Command.

**02:05 AM — The Root Cause**
You write an eBPF script to trace kernel block I/O latency. The script instantly reveals that a rogue backup job is saturating the SAN storage array for 5 seconds every hour. The database is healthy, but it is waiting on the physical disk hardware.

**02:08 AM — Immediate Containment**
You use `cgroups` to throttle the I/O of the backup job, returning latency to normal without killing the backup.

**02:15 AM — Documentation**
You resolve the P1 incident, draft the Blameless Post-Mortem timeline, and assign an Action Item to move the backup job to a dedicated Read-Replica.

**02:20 AM — Coffee**
You step away from the terminal. The outage was mitigated purely by deep kernel visibility and calm incident command.
...
This book is **Volume 5: Senior Engineer**. It is the final entry in the 5-volume *Linux Support Engineer Master Handbook* series. 
