...
> [!TIP] Support Engineer Tip #23
> **The SSH Lockout:** The most common mistake a junior engineer makes is typing `ufw enable` before `ufw allow 22`. If you do this on a remote server, your SSH connection will instantly freeze and drop, and you will be permanently locked out. You will have to log into your cloud provider's web console to fix it via VNC.
...
> [!IMPORTANT] Incident Report: The Silent Drop
>
> **Problem:** End User (Dave): "I just installed a web server on Ubuntu. When I type `ss -tulpn`, I clearly see it listening on Port 80. But when I type my IP address into my web browser on my laptop, it times out!"
>
> **Investigation:** Charlie knows that if a service is listening internally but timing out externally, it is almost always blocked at the perimeter.
>
> **Wrong Assumption:** Bob (Junior Admin) says: "The Nginx configuration file must be broken. Let's uninstall and reinstall the web server."
>
> **Root Cause:** Alice (Senior Admin) intervenes. The server is fine. The firewall is simply ignoring the customer's request.
>
> **Lessons Learned:** Alice logs in and checks the firewall status.
> 
> ```bash
> alice@prod-web1:~$ sudo ufw status
> Status: active
> 
> To                         Action      From
> --                         ------      ----
> 22/tcp                     ALLOW       Anywhere
> alice@prod-web1:~$ sudo ufw allow 80/tcp
> Rule added
> alice@prod-web1:~$ sudo ufw status
> Status: active
> 
...
> ```
> 
> Alice runs `sudo ufw allow 80/tcp`. The customer refreshes their browser and the website loads instantly. Always check the firewall before blaming the application!
## Hands-on Lab
