...
> [!TIP] Support Engineer Tip #9
> **apt vs apt-get**: You will see both online. `apt-get` is the older, low-level scripting tool. `apt` is the newer, user-friendly wrapper that adds color, progress bars, and merges the most common commands. For daily troubleshooting, use `apt`. For writing automated bash scripts, use `apt-get` or `apt-get -y`.
...
> [!IMPORTANT] Incident Report: The Missing Dependency
>
> **Problem:** End User (Dave): "I am trying to run a deployment script, but it keeps failing with 'curl: command not found'. Please fix it."
>
> **Investigation:** Charlie logs into the newly provisioned server and confirms the binary is missing.
> 
> ```bash
> charlie@prod-web1:~$ curl https://api.github.com
> -bash: curl: command not found
> charlie@prod-web1:~$ sudo apt install curl
> Reading package lists... Done
> E: Unable to locate package curl
> ```
>
> **Evidence:** The system does not have `curl` installed, AND the package manager cannot find it in its database.
>
> **Wrong Assumption:** Bob (Junior Admin) says: "The repository must be down, let's download the source code and compile it."
>
> **Root Cause:** Alice (Senior Admin) realizes that this is a fresh server that was just booted. The local `apt` cache is completely empty. The system doesn't know where to download `curl` from because it hasn't downloaded the index yet.
>
> **Lessons Learned:** Alice runs `sudo apt update` to pull the latest index from the Ubuntu servers. Then, she runs `sudo apt install curl`. It works instantly. Always update the cache before attempting to install software on a new machine.
## Hands-on Lab
