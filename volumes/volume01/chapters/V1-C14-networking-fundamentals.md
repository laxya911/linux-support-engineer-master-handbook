...
> [!TIP] Support Engineer Tip #13
> **Don't install `net-tools`:** If you type `ifconfig` and get `command not found`, do not install the `net-tools` package. Force yourself to learn `ip a`. It provides much more accurate information about secondary IPs and modern routing configurations.
...
> [!IMPORTANT] Incident Report: The Deaf Server
>
> **Problem:** End User (Dave): "I just installed Nginx, but when I go to my server's IP address in a web browser, it says Connection Refused."
>
> **Investigation:** Charlie knows that "Connection Refused" usually means the network packet reached the server perfectly fine, but no software was actively listening on that specific port (80) to accept it.
> 
> ```bash
> charlie@prod-web1:~$ sudo ss -tulpn | grep 80
> charlie@prod-web1:~$ 
> ```
>
> **Evidence:** The output is completely blank. Nothing on the server is listening on port 80.
>
> **Wrong Assumption:** Bob (Junior Admin) says: "The firewall is blocking port 80. Let's disable the firewall."
>
> **Root Cause:** Alice (Senior Admin) intervenes. A firewall block usually results in a "Connection Timeout" (the packet is silently dropped). "Connection Refused" is an active rejection by the OS because no process is home. She checks the service.
> 
> ```bash
...
> ```
>
> **Lessons Learned:** Alice sees that Nginx crashed on startup due to a configuration error. She fixes the configuration file, runs `systemctl start nginx`, and runs `ss -tulpn | grep 80` again. This time it shows Nginx listening on `:80`. The website loads instantly. Do not blame the firewall when the service itself is dead.
## Hands-on Lab
