...
> [!TIP] Support Engineer Tip #21
> **Testing Cron Jobs:** Never schedule a cron job for tomorrow and assume it will work. Always schedule it to run 2 minutes from *now* (e.g., if it is 14:30, set it to `32 14 * * *`) and watch the logs to prove it actually executes successfully in the restricted cron environment.
...
> [!IMPORTANT] Incident Report: The Sunday Crash
>
> **Problem:** End User (Dave): "Our application fills up the `/tmp/app_cache` directory constantly. Every Monday morning we come in and the server is crashed because the disk is full."
>
> **Investigation:** Charlie reviews the disk usage and confirms the cache directory is the culprit.
>
> **Wrong Assumption:** Bob (Junior Admin) says: "I will set an alarm on my phone to log in every Sunday night and delete the files manually."
>
> **Root Cause:** Alice (Senior Admin) intervenes. Humans forget. Machines do not. This must be automated via `cron`.
>
> **Lessons Learned:** Alice writes a cache cleaner script using absolute paths and schedules it to run every Sunday at midnight.
> 
> ```bash
> alice@prod-app1:~$ cat /opt/scripts/clear_cache.sh
> #!/bin/bash
> /bin/rm -rf /tmp/app_cache/*
> /bin/echo "Cache cleared on $(date)" >> /var/log/cache_cleaner.log
> alice@prod-app1:~$ chmod +x /opt/scripts/clear_cache.sh
> alice@prod-app1:~$ crontab -l
> # Run at 00:00 (Midnight) only on Day 0 (Sunday)
> 0 0 * * 0 /opt/scripts/clear_cache.sh
> ```
> 
> The server never crashes again, and the team reclaims their Monday mornings.
