...
> [!TIP] Support Engineer Tip #15
> **Archiving is not Compression:** `tar` does *not* save disk space. If you use `tar` to bundle 100MB of text files, the resulting `.tar` file will be exactly 100MB. To shrink the size, you must combine `tar` with a compression tool.
...
> [!IMPORTANT] Incident Report: The Massive Download
>
> **Problem:** End User (Dave): "Our developers need to download the Apache web server logs to analyze a bug. There are 4,500 individual log files in the directory. Downloading them via SFTP is taking hours."
>
> **Investigation:** Charlie reviews the directory and confirms there are thousands of small text files. Downloading them over a network individually is incredibly slow because of the TCP handshake overhead required for each file.
> 
> ```bash
> charlie@prod-web1:~$ ls -1 /var/log/httpd | wc -l
> 4500
> charlie@prod-web1:~$ du -sh /var/log/httpd
> 5.2G    /var/log/httpd
> ```
>
> **Evidence:** There are 4,500 files consuming 5.2GB of space.
>
> **Wrong Assumption:** Bob (Junior Admin) says: "Let's compress them into a `.zip` file using the `zip` command." (Linux servers rarely have `zip` installed by default).
>
> **Root Cause:** Alice (Senior Admin) intervenes. The native, most efficient way to handle this on Linux is `tar` combined with `gzip`.
>
> **Lessons Learned:** Alice runs `sudo tar -czvf /tmp/apache_logs.tar.gz /var/log/httpd/`. She bundles all 4,500 files into a single `.tar.gz` archive. Because text logs compress exceptionally well, the 5.2GB of logs shrinks down to a 300MB file. The developer downloads the single file in 15 seconds.
## Hands-on Lab
